...
const IndiaPackagesPage = () => {
  ...
  const handleInquireClick = (pkgId: string) => {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {domesticPackages.map((pkg) => (
            ...
            >
              ...
              {/* Price & CTA */}
              <div className="px-1 pt-2 flex justify-between items-center border-t border-black/[0.04]">
                <div className="flex flex-col">
                  <span className="text-[10px] font-bold text-primary/40 uppercase tracking-wide">Starting from</span>
                  <span className="text-xl font-extrabold text-[#3b7c74]">{pkg.price}</span>
                </div>
                <button
                  onClick={() => handleInquireClick(pkg.id)}
                  className="px-5 py-2.5 bg-primary hover:bg-[#123632] text-white rounded-full text-xs font-bold transition-all shadow-sm hover:shadow cursor-pointer flex items-center gap-1.5"
                >
